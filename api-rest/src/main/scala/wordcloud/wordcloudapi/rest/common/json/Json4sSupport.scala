package wordcloud.wordcloudapi.rest.common.json
...
import org.json4s.{ DefaultFormats, Formats, MappingException, Serialization, jackson }
...
trait SnakeCaseJson4sJacksonSupport extends Json4sJacksonSupport {
  implicit val snakeCaseEnabled = Json4sSupport.SnakeCase.Enabled
}

/**
 * Automatic to and from JSON marshalling/unmarshalling using an in-scope *Json4s* protocol.
 *
 * Snake case is enabled if an implicit [[Json4sSupport.SnakeCase.Enabled]] is in scope.
 */
